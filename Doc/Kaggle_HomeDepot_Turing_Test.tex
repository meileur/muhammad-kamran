...
\abstract{In the Home Depot Product Search Relevance Challenge, we were asked to build a model to predict the relevance score of search results, given the searching queries, resulting product titles and product descriptions. For some of the products, we are also provided with a set of attributes. This document describes our team's solution, which relies heavily on feature engineering and model ensembling.}
...
%%% this is a section in the Kaggle template https://www.kaggle.com/wiki/WinningModelDocumentationTemplate
\section{Background}
\begin{itemize}
...
\begin{comment}
 (Note that due to randomness in the hyperopt routine, the reproduced ``Best Ensemble Model from Chenglong'' is of a similar score with the actual submission during competition. Thus also the ``Best Final Ensemble Model'' which rely on that submission.)
 \end{comment}
\begin{table}[t]
\centering
